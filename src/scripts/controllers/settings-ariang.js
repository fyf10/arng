(function () {
    ...
    angular.module('ariaNg').controller('AriaNgSettingsController', ['$rootScope', '$scope', '$routeParams', '$window', '$interval', '$timeout', '$filter', 'clipboard', 'ariaNgLanguages', 'ariaNgCommonService', 'ariaNgVersionService', 'ariaNgKeyboardService', 'ariaNgNotificationService', 'ariaNgLocalizationService', 'ariaNgLogService', 'ariaNgFileService', 'ariaNgSettingService', 'ariaNgMonitorService', 'ariaNgTitleService', 'ariaNgTrackerService', 'aria2SettingService', function ($rootScope, $scope, $routeParams, $window, $interval, $timeout, $filter, clipboard, ariaNgLanguages, ariaNgCommonService, ariaNgVersionService, ariaNgKeyboardService, ariaNgNotificationService, ariaNgLocalizationService, ariaNgLogService, ariaNgFileService, ariaNgSettingService, ariaNgMonitorService, ariaNgTitleService, ariaNgTrackerService, aria2SettingService) {
        ...
        $scope.context = {
            ...
            trackerLastUpdateTime: ariaNgSettingService.getTrackerLastUpdateTime()
        };
        ...
        $scope.setTrackerAutoUpdate = function (value) {
            ariaNgSettingService.setTrackerAutoUpdate(value);

            if (value) {
                ariaNgTrackerService.checkAndStartAutoUpdate();
            }
        };

        $scope.setTrackerAutoUpdateInterval = function (value) {
            ariaNgSettingService.setTrackerAutoUpdateInterval(value);
        ...
        $scope.addTrackerSource = function () {
            var sourceUrl = $scope.context.newTrackerSource;
            ...
            if (!/^https?:\/\/.+/.test(sourceUrl)) {
                ariaNgCommonService.showError('Invalid tracker source URL');
                return;
            }

            // Check if already exists
            if ($scope.context.settings.trackerSources.indexOf(sourceUrl) !== -1) {
                ariaNgCommonService.showError('Tracker source already exists');
                return;
            }

            if (ariaNgSettingService.addTrackerSource(sourceUrl)) {
                $scope.context.settings.trackerSources = ariaNgSettingService.getTrackerSources();
                $scope.context.newTrackerSource = '';
            }
        };

        $scope.removeTrackerSource = function (index) {
            if (index >= 0 && index < $scope.context.settings.trackerSources.length) {
                var sourceUrl = $scope.context.settings.trackerSources[index];
                if (ariaNgSettingService.removeTrackerSource(sourceUrl)) {
                    $scope.context.settings.trackerSources = ariaNgSettingService.getTrackerSources();
                }
            }
        };

        $scope.updateTrackerSource = function (index, sourceUrl) {
            if (index >= 0 && index < $scope.context.settings.trackerSources.length) {
                sourceUrl = (sourceUrl || '').trim();
                ...
                $scope.context.settings.trackerSources[index] = sourceUrl;
                ariaNgSettingService.setTrackerSources($scope.context.settings.trackerSources);
            }
        };

        $scope.updateTrackersManually = function () {
            if ($scope.context.isUpdatingTrackers) {
                return;
            }

            if (!$scope.context.settings.trackerSources || $scope.context.settings.trackerSources.length === 0) {
                ariaNgCommonService.showError('Please add at least one tracker source');
                return;
            }

            $scope.context.isUpdatingTrackers = true;

            ariaNgTrackerService.updateTrackers().then(function () {
                ariaNgCommonService.showOperationSucceeded('Trackers updated successfully');
                $scope.context.trackerLastUpdateTime = ariaNgSettingService.getTrackerLastUpdateTime();
            }).catch(function (error) {
                ariaNgCommonService.showError('Failed to update trackers: ' + (error.message || error));
            }).finally(function () {
                $scope.context.isUpdatingTrackers = false;
            });
        };
