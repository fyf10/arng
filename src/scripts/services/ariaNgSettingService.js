(function () {
    ...
    angular.module('ariaNg').factory('ariaNgSettingService', ['$window', '$location', '$filter', 'ariaNgConstants', 'ariaNgDefaultOptions', 'ariaNgLanguages', 'ariaNgCommonService', 'ariaNgLogService', 'ariaNgStorageService', function ($window, $location, $filter, ariaNgConstants, ariaNgDefaultOptions, ariaNgLanguages, ariaNgCommonService, ariaNgLogService, ariaNgStorageService) {
        ...
        return {
            ...
            getTrackerAutoUpdate: function () {
                return getOption('trackerAutoUpdate');
            },
            setTrackerAutoUpdate: function (value) {
                setOption('trackerAutoUpdate', value);
            },
            getTrackerAutoUpdateInterval: function () {
                var value = getOption('trackerAutoUpdateInterval');
                return value || '1d';
            },
            setTrackerAutoUpdateInterval: function (value) {
                setOption('trackerAutoUpdateInterval', value);
            },
            getTrackerSources: function () {
                var value = getOption('trackerSources');
                return angular.isArray(value) ? value : [];
            },
            setTrackerSources: function (value) {
                setOption('trackerSources', angular.isArray(value) ? value : []);
            },
            addTrackerSource: function (sourceUrl) {
                var sources = this.getTrackerSources();
                if (sources.indexOf(sourceUrl) === -1) {
                    sources.push(sourceUrl);
                    this.setTrackerSources(sources);
                    return true;
                }
                return false;
            },
            removeTrackerSource: function (sourceUrl) {
                var sources = this.getTrackerSources();
                var index = sources.indexOf(sourceUrl);
                if (index !== -1) {
            ...
            getTrackerLastUpdateTime: function () {
                return getOption('trackerLastUpdateTime');
            },
            setTrackerLastUpdateTime: function (value) {
                setOption('trackerLastUpdateTime', value);
            },
