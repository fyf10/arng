    ...
    angular.module('ariaNg').factory('ariaNgCommonService', ['$window', '$location', '$timeout', 'base64', 'moment', 'SweetAlert', 'ariaNgConstants', 'ariaNgLocalizationService', function ($window, $location, $timeout, base64, moment, SweetAlert, ariaNgConstants, ariaNgLocalizationService) {
        ...
        return {
            ...
            getLongTimeFromUnixTime: function (unixTime) {
                // 确保使用本地时区（上海时区）
                return moment(unixTime, 'X').utcOffset(8 * 60).format('HH:mm:ss');
            },
            isUnixTimeAfter: function (datetime, duration, unit) {
                // 使用上海时区进行时间比较
                return moment(datetime, 'X').utcOffset(8 * 60).isAfter(moment().utcOffset(8 * 60).add(duration, unit));
            },
            formatDateTime: function (datetime, format) {
                // 确保使用本地时区（上海时区）
                return moment(datetime).utcOffset(8 * 60).format(format);
            },
